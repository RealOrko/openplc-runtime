...
# On Cygwin/MSYS2, the PE/COFF linker requires all symbols resolved at link time
# when creating shared libraries. The PLC shared library references functions
# provided by the plc_main executable (e.g. TCP communication blocks). On Linux
# these resolve automatically at dlopen time via -rdynamic. On Cygwin/MSYS2, we
# generate an import library from plc_main.exe so the linker can resolve these
# symbols at link time (they bind to plc_main.exe at runtime).
LINK_FLAGS=""
case "$(uname -s)" in
    CYGWIN*|MSYS*|MINGW*)
        if [ -f "$BUILD_PATH/plc_main.exe" ]; then
            echo "[INFO] Generating import library for plc_main..."
            {
                echo "LIBRARY plc_main.exe"
                echo "EXPORTS"
                nm "$BUILD_PATH/plc_main.exe" 2>/dev/null | awk '/^[0-9a-fA-F]+ T /{print $3}'
            } > "$BUILD_PATH/plc_main.def"
            dlltool -d "$BUILD_PATH/plc_main.def" -l "$BUILD_PATH/libplc_main.a"
            rm -f "$BUILD_PATH/plc_main.def"
            LINK_FLAGS="$BUILD_PATH/libplc_main.a"
        fi
        ;;
esac
