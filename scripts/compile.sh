...
# must link against the import library generated by CMake (--out-implib) so the
# linker can resolve these symbols against plc_main.
LINK_FLAGS=""
case "$(uname -s)" in
    CYGWIN*|MSYS*|MINGW*)
        if [ -f "$BUILD_PATH/libplc_main.a" ]; then
